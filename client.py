...
def recivedMsg():
    ...
    while True:
        ...
        if('player_type' in message):
            ...
            # name = recvMsg['player_name']
            # if(playerType == 'player1'):
            #     canvas2.create_text(400,  screen_height/2 + 100, text = name, font=("Chalkboard SE",80), fill='#fff176' )
            # else:
            #     canvas2.create_text(screen_width - 300, screen_height/2 + 100, text = name, font=("Chalkboard SE",80), fill='#fff176' )
        elif('⚀' in message):
            # Dice with value 1
            canvas2.itemconfigure(dice, text='\u2680')
        elif('⚁' in message):
